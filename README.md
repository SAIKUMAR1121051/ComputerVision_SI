...
2. This API has only one post route for the above-mentioned endpoint. Any image URL can be passed as a body parameter using the name ‘imageUrl’.
...
==> Open postman and use the above-mentioned API URL. 
==> Select the request type to ‘POST’


==> Click on the ‘body’ as shown in the above image and select the data type as ‘JSON’.
==> Enter the json text with ‘imageUrl’ as parameter and https://upload.wikimedia.org/wikipedia/commons/3/3c/Shaki_waterfall.jpg as an value to the parameter.
==> Send the request.
...
2. Use the URL http://159.65.32.185:3000/api/v1/user/analyzeImage as a post request to analyze an image. To this request, the generated token in the step 1has to be passed as an ‘Authorization’ parameter in the header. Also, pass the imageUrl as a body parameter and send the request.
